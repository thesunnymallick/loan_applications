  ...
  const columns = [
...

    {
      ...
      render: (text) => {
        ...
        const loanType = loanTypeMapping[text] || {
          label: text,
          color: "#595959",
        };
...

    {
      ...
      render: (record) => (
        <Button
          onClick={() =>
            handleNavigateUploadDocPage(record.loan, record.file_no)
          }
          type="link"
          icon={<EyeOutlined />}
        />
                  ...
                  <div>
                    {loan.loanRange && (
                      <p className="text-lg mb-4">
                        Loan Range:{" "}
                        <span className="font-semibold">{loan.loanRange}</span>
                      </p>
                    )}
